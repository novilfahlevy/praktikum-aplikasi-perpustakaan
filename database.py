...
class Database :
	...
	def buat_tabel(self, seed=False, truncate=False) :
		...
		if truncate : cursor.execute('DROP TABLE IF EXISTS `pengguna`;')
		if truncate : cursor.execute('DROP TABLE IF EXISTS `penerbit`;')
		if truncate : cursor.execute('DROP TABLE IF EXISTS `pengadaan`;')
		if truncate : cursor.execute('DROP TABLE IF EXISTS `buku_pengadaan`;')
		if truncate : cursor.execute('DROP TABLE IF EXISTS `buku`;')
		if truncate : cursor.execute('DROP TABLE IF EXISTS `peminjaman`;')
		...
		cursor.execute('ALTER TABLE `buku_pengadaan` ADD FOREIGN KEY (`kode_pengadaan`) REFERENCES `pengadaan` (`kode`) ON DELETE CASCADE;')
